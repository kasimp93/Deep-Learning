...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # Mini-Batch Mean
        mu_beta = 1/float(N) * np.sum(x, axis=0)
        
        # Mini-Batch Variance
        term1 = (x-mu_beta)
        term2 = term1**2
        var_beta = 1/float(N) * np.sum(term2,axis=0)
        
        #Normalize
        root_sig =  np.sqrt((var_beta)+eps)
        invvar = 1. / root_sig
        x_i = term1/root_sig
        ...
        running_mean = momentum * running_mean + (1 - momentum) * mu_beta
        running_var = momentum * running_var + (1 - momentum) * var_beta
        cache = (mu_beta, term1, term2, var_beta, invvar, root_sig, x_i, gamma_xi, running_mean, running_var, gamma, beta, x, bn_param)
        ...
        # Mini-Batch Mean
        mu_beta = running_mean
        
        # Mini-Batch Variance
        var_beta =  running_var
        
        #Normalize
        root_sig =  np.sqrt((var_beta)+eps)
        x_i = (x - mu_beta)/root_sig
        ...
        running_mean = momentum * running_mean + (1 - momentum) * mu_beta
        running_var = momentum * running_var + (1 - momentum) * var_beta
        cache = (mu_beta, var_beta, root_sig, x_i, gamma_xi, running_mean, running_var, gamma, beta, x, bn_param)
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
    else:
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    mu_beta, term1, term2, var_beta, invvar, root_sig, x_i, gamma_xi, running_mean, running_var, gamma, beta, x, bn_param = cache
    ...
    # Backprop step 8
    dva2 = gamma * dva3
    dgamma = np.sum(x_i * dva3, axis=0)

    # Backprop step 7
    dxmu = invvar * dva2
    dinvvar = np.sum(term1 * dva2, axis=0)

    # Backprop step 6
    dsqrtvar = -1. / (root_sig**2) * dinvvar

    # Backprop step 5
    dvar = 0.5 * (var_beta + eps)**(-0.5) * dsqrtvar

    # Backprop step 4
    dcarre = 1 / float(N) * np.ones((term2.shape)) * dvar
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = None # first dropout mask. Notice /p!
        out = x
        pass
        #######################################################################
        #                            END OF YOUR CODE                         #
