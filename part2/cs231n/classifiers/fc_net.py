...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['W2'] = std * np.random.randn(hidden_dim, output_size)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):            
            self.params['W%d' % (i+1)] = np.random.normal(0, std, (dims[i], dims[i + 1]))
    ...
    def loss(self, X, y=None):
        ...
        cache_hidden_layer = {}   
        dropout_cache = {}
        cache_bn_hidden_layer = {}     
        
        for i in range(1,self.num_layers):
            if not self.use_batchnorm:
                hidden_layer[i], cache_hidden_layer[i] = affine_relu_forward(hidden_layer[i-1], self.params['W%d' % i], self.params['b%d' % i])
            else:
                hidden_layer[i], cache_bn_hidden_layer[i] = affine_bn_relu_forward(hidden_layer[i-1], self.params['W%d' % i], self.params['b%d' % i], self.params['gamma%d' % i], self.params['beta%d' % i], self.bn_params[i-1] )
                
            if self.use_dropout:
                hidden_layer[i], dropout_cache[i] = dropout_forward(hidden_layer[i-1], self.dropout_param)
        # Forward into second layer
        scores, cache_scores = affine_forward(hidden_layer[self.num_layers-1], self.params['W%d' % self.num_layers], self.params['b%d' % self.num_layers])
        ...
        #last layer:
        affine_dx, affine_dw, affine_db = affine_backward(dscores, cache_scores)
        ...
        for i in range(self.num_layers-1,0,-1):
            ...
            if not self.use_batchnorm:       
                affine_dx, affine_dw, affine_db = affine_relu_backward(affine_dx, cache_hidden_layer[i])
    
            else:       
                affine_dx, affine_dw, affine_db, dgamma, dbeta = affine_bn_relu_backward(affine_dx, cache_bn_hidden_layer[i]) 
...
#            grads['gamma' + str(self.num_layers)]=1 ##self.num_layers or direct number of total layers. wasn't working in the other one
